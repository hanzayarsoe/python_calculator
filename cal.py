...
class Calculator(tk.Tk):
    def __init__(self):
        ...
        self.input_field = tk.Entry(
            self, textvariable=self.input_text, justify="right", width=30)
        ...
        # add memory button
        tk.Button(self, text="M+", width=5,
                  command=self.memory_add).grid(row=5, column=0, padx=5, pady=5)
        tk.Button(self, text="M-", command=self.memory_substract).grid(
            row=5,column=1, padx=5, pady=5)
        tk.Button(self, text="MR", command=self.memory_recall).grid(
            row=5, column=2, padx=5, pady=5)
        tk.Button(self, text="MC", command=self.memory_clear).grid(
            row=5, column=3, padx=5, pady=5)

        # add  clean and delete button
        tk.Button(self, text="C", width=5, command=self.clear_input).grid(
            row=6, column=0, padx=5, pady=5)
        tk.Button(self, text="DEL", width=5, command=self.delete_char).grid(
    ...
    def button_click(self, button):
        ...
        if button == '=':
            ...
            self.input_text.set(math.sin(float(self.input_text.get())))
        elif button == 'cos':
            self.input_text.set(math.cos(float(self.input_text.get())))
        elif button == 'tan':
            self.input_text.set(math.tan(float(self.input_text.get())))
        elif button == 'sqrt':
    ...
    def delete_char(self):
        # delete the last char from the input field
        current_text = self.input_text.get()
